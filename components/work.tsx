...
export const WorkContainer: React.FC<IAuth> = ({ children }) => {
  return (
...
export const WorkLeft: React.FC<IAuth> = ({ children, progress }) => {
  let translateY = Math.max(0, 50 - progress * 3 * 50);
  if (progress > 0) translateY = Math.max(-50, -(progress - 0.85) * 2 * 50);
  return (
    <div
      className="flex flex-col items-center justify-center text-3xl lg:text-3xl h-[30vh] lg:h-auto"
      style={{ transform: `translateY(${translateY}px)` }}
    >
      <div className="laeding-10">{children}</div>
    </div>
  );
};

export const WorkRight: React.FC<IAuth> = ({ children, progress }) => {
...
export const WorkLink: React.FC<LinkProps> = ({ children }) => (
  <Link
    href="/"
    target="_blank"
    rel="noreferrer"
